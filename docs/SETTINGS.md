...
The Theme integration does not create or select a `SiteLayout`. A later layout renderer must still require an enabled Site mode and an explicitly selected persisted `SiteLayout` before composing public regions; otherwise the existing Nuxt public shell remains active. Theme resources, menu resources, and Nuxt application layouts remain separate contracts.
