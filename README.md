...
To choose the initial value yourself for a terminal deployment, copy `.dev.vars.example` to `.dev.vars`, uncomment `NUXT_AUTH_SECRET`, and pass `--secrets-file .dev.vars`. For a new Worker or one missing the secret, a caller-supplied secrets file must contain a non-empty `NUXT_AUTH_SECRET`; the wrapper never adds a second conflicting secrets file. Including `NUXT_AUTH_SECRET` in a supplied file for an existing Worker intentionally replaces the secret during that deployment.
