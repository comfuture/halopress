...
describe('page editor workspace contracts', () => {
  ...
  it('uses a full-bleed desktop canvas and framed tablet and mobile canvases', async () => {
    ...
    expect(editor).toContain('isFullWidthCanvas.value ? \'rounded-none\' : \'rounded-t-md\'')
    ...
    expect(editor).toContain('min-h-12 flex-wrap border-b border-muted bg-default/95 px-2 py-2')
  ...
  it('keeps Block Library and Inspector in one ordered right panel', async () => {
    ...
    expect(editor).toContain('if (!isEditMode.value || panelCollapsed.value) return \'xl:grid-cols-1\'')
  ...
  it('uses one icon-only DashboardToolbar toggle for the desktop panel and small-screen drawer', async () => {
    ...
    expect(rightSlot).toContain('aria-label="Canvas viewport"')
    ...
    expect(editor).toContain('window.matchMedia(\'(min-width: 1280px)\').matches')
