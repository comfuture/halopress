...
describe('page editor workspace contracts', () => {
  ...
  it('offers explicit blank and reviewed starter choices only on new pages', async () => {
    ...
    expect(createPage).toContain('const hasStarterChoice = computed(() => starterChoice.value !== null)')
    expect(createPage).toContain('hasStarterChoice.value && isDirty.value')
    expect(createPage).toContain('if (!canSaveDraft.value) return')
    expect(createPage).toContain('if (!canPublish.value) return')
