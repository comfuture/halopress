...
definePageMeta({
  key: route => `${String(route.params.schema)}/${String(route.params.id)}`
})
...
if (schemaKey.value === PUBLIC_PAGE_ROUTE_PREFIX) {
  ...
  if (error.value && statusCode !== 404) {
    throw createError({
      ...
      statusMessage: 'Unable to load page'
    })
  }
...
if (!standalonePage.value) {
  ...
  doc.value = contentResult.content.value
  schema.value = contentResult.schema.value
}

useSeoMeta({
  title: () => standalonePage.value?.title || undefined
})
